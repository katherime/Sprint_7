...
class TestAcceptOrder:
    ...
    @allure.title('Проверка, что заказ можно принять и успешный запрос возвратится корректно')
    ...
    def test_successful_accept_order_without_id_courier(self, login_pass):
        ...
        assert response.status_code == 400 and response.json()[
            "message"] == TextsErrors.response_not_enough_data_for_search

    @allure.title('Проверка, что если передать неверный id курьера, запрос вернёт ошибку')
    def test_successful_accept_order_with_wrong_id_courier(self, login_pass):
        order_methods = OrderMethods()
        create_response = order_methods.create_new_order(right_data_for_order_creation)
        id_order = create_response.json()["track"]
        response = order_methods.accept_order(id_order, "1")
        assert response.status_code == 404 and response.json()["message"] == TextsErrors.response_id_courier_not_exist

    @allure.title('Проверка, что если не передать id заказа, запрос вернёт ошибку;')
    ...
    @allure.title('Проверка, что если передать неверный id заказа, запрос вернёт ошибку')
    def test_successful_accept_order_with_wrong_id_order(self, login_pass):
        ...
        assert response.status_code == 404 and response.json()["message"] == TextsErrors.response_id_order_not_exist
